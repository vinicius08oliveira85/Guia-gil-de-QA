...
describe('JiraMediaService', () => {
    ...
    describe('detectMediaType', () => {
        ...
        it('deve detectar documentos Word', () => {
            ...
            expect(jiraMediaService.detectMediaType('arquivo', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document')).toBe('document');
        });
        ...
        it('deve detectar planilhas Excel', () => {
            ...
            expect(jiraMediaService.detectMediaType('arquivo', 'application/vnd.ms-excel')).toBe('spreadsheet');
        });
    ...
    describe('resolveMediaUrl', () => {
        it('deve resolver URL quando config do Jira está disponível', () => {
            ...
            const url = jiraMediaService.resolveMediaUrl('123', 'file.pdf', undefined, { useProxy: false });
            expect(url).toBe('https://jira.example.com/secure/attachment/123/file.pdf');
        });

        it('deve usar jiraUrl fornecido quando disponível', () => {
            const url = jiraMediaService.resolveMediaUrl('123', 'file.pdf', 'https://custom-jira.com', { useProxy: false });
        ...
        it('deve codificar nome do arquivo corretamente', () => {
            ...
            const url = jiraMediaService.resolveMediaUrl('123', 'arquivo com espaços.pdf', undefined, { useProxy: false });
    ...
    describe('isJiraUrl', () => {
        it('deve identificar URLs do Jira por origin', () => {
            ...
            expect(jiraMediaService.isJiraUrl('https://jira.example.com/secure/attachment/123/file.pdf')).toBe(true);
        });

        it('deve identificar URLs do Jira por path', () => {
            expect(jiraMediaService.isJiraUrl('https://any-domain.com/secure/attachment/123/file.pdf')).toBe(true);
