...
export const solusSchema: ApiTableSchema[] = [
  {
    ...
    fields: [
      ...
      { name: 'hospital', type: 'string', description: 'Nome do hospital atual' },
      {
        name: 'programa',
        type: 'string',
        description: 'Programa de saúde vinculado',
        example: 'Mãe Curitibana',
      },
      {
        name: 'leito',
        type: 'string',
        description: 'Identificador do leito atual',
        example: '101A',
      },
      ...
      { name: 'alta_inicio', type: 'date|null', description: 'Data prevista de alta' },
      {
        name: 'alta_fim',
        type: 'date|null',
        description: 'Data de alta efetiva (null = internado)',
      },
      {
        name: 'criticidade_tipo',
        type: 'number',
        description: 'Nível de criticidade (0-3)',
        example: '2',
      },
  ...
  },
  {
    ...
    fields: [
      ...
      { name: 'event_date', type: 'timestamp', description: 'Data/hora do evento' },
      {
        name: 'event_type',
        type: 'string',
        description: 'Tipo de evento (transferencia, alta, observacao)',
      },
  ...
  },
  {
    ...
    fields: [
      ...
      { name: 'patient_id', type: 'number', description: 'Paciente vinculado' },
      {
        name: 'espera_cirurgia',
        type: 'boolean',
        description: 'Flag indicando se está aguardando',
        example: 'true',
      },
      ...
      { name: 'aguarda_opme', type: 'boolean', description: 'Indica se depende de OPME' },
      {
        name: 'status_atual',
        type: 'string',
        description: 'Status atual (vide lista)',
  ...
  },
  {
    ...
    primaryKey: 'id',
    relations: ['patients'],
    statuses: ['aguardando_solicitacao', 'solicitado', 'em_analise', 'respondido', 'concluido'],
    fields: [
      { name: 'id', type: 'number', description: 'Identificador' },
      { name: 'patient_id', type: 'number', description: 'Paciente vinculado' },
  ...
  },
  {
    ...
    fields: [
      ...
      { name: 'data_inicio_espera', type: 'date', description: 'Início do processo' },
      {
        ...
        type: 'string',
        description: 'Serviços necessários após alta',
      },
      { name: 'status_atual', type: 'string', description: 'Status atual (vide lista)' },
