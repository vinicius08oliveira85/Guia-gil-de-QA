...
): { completed: number; total: number; required: number; requiredCompleted: number } => {
...
export const canMoveToNextPhase = (
  checklist: ChecklistItem[]
): { canMove: boolean; missingRequired: string[] } => {
...
export const updateChecklistItem = (
  ...
  return checklist.map(item => (item.id === itemId ? { ...item, ...updates } : item));
};
