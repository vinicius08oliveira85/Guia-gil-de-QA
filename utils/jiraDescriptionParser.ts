...
function adfNodeToHTML(
  ...
  if (node.content && Array.isArray(node.content)) {
    const content = node.content
      .map(child => adfNodeToHTML(child, jiraUrl, jiraAttachments, depth + 1))
      .join('');
      ...
      case 'media': {
        ...
        if (jiraAttachments && jiraUrl) {
          ...
          if (!attachment && url) {
            ...
            attachment = jiraAttachments.find(
...
export function parseJiraDescriptionHTML(
  ...
  if (typeof description === 'string') {
    ...
    if (description.type === 'doc' && Array.isArray(description.content)) {
      ...
      html = sanitizeHTML(
        description.map(node => adfNodeToHTML(node, jiraUrl, jiraAttachments)).join('')
      );
