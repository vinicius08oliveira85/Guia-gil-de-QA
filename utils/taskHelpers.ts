...
type ProjectWithJiraStatuses =
  | { settings?: { jiraStatuses?: Array<{ name: string; color?: string } | string> } }
  | null
  | undefined;

type ProjectWithJiraPriorities =
  | { settings?: { jiraPriorities?: Array<{ name: string } | string> } }
  | null
  | undefined;
 ...
 */
export const getDisplayStatusLabel = (
  task: JiraTask,
  project?: ProjectWithJiraStatuses
): string => {
 ...
 */
export const getDisplayPriorityLabel = (
  task: JiraTask,
  project?: ProjectWithJiraPriorities
): string => {
...
export const getTestPhaseStatus = (
  ...
  if (task.type === 'Epic' || task.type === 'História') {
    ...
    const allSubtasksCompleted = subtasks.every(
      subtask => getTestPhaseStatus(subtask, allTasks) === 'Concluído'
    );
