...
export const generateSuggestions = (project: Project): Suggestion[] => {
    ...
    if (tasks.length === 0) {
        suggestions.push({
            ...
            message: 'Adicione uma tarefa para começar a organizar seu trabalho de QA. Use o wizard para aprender passo a passo!',
            priority: 'high'
        });
    ...
    const tasksWithoutBDD = tasks.filter(t => 
        t.status !== 'Done' && 
    ...
    if (tasksWithoutBDD.length > 0) {
        suggestions.push({
            ...
            message: 'Cenários BDD ajudam a definir o comportamento esperado. Considere criar cenários BDD para suas tarefas em andamento.',
            priority: 'medium'
        });
    }

    // Sugestão: Tarefas sem casos de teste
    const tasksWithoutTests = tasks.filter(t => 
        t.status !== 'Done' && 
    ...
    if (tasksWithoutTests.length > 0) {
        suggestions.push({
            ...
            message: 'Casos de teste são essenciais para validar funcionalidades. Gere casos de teste para suas tarefas.',
            priority: 'high'
        });
    }
    ...
    if (unexecutedTests.length > 0) {
        suggestions.push({
            ...
            message: 'Execute seus casos de teste para validar as funcionalidades e identificar problemas.',
            priority: 'medium'
        });
    }
    ...
    if (criticalBugs.length > 0) {
        suggestions.push({
            ...
            message: 'Bugs críticos devem ser corrigidos com prioridade. Revise e corrija esses bugs urgentemente.',
            priority: 'high'
        });
    }

    // Sugestão: Tarefas concluídas sem testes passando
    const doneTasksWithoutPassingTests = tasks.filter(t => 
        t.status === 'Done' && 
        t.testCases && 
    ...
    if (doneTasksWithoutPassingTests.length > 0) {
        suggestions.push({
            ...
            message: 'Tarefas concluídas devem ter pelo menos alguns testes passando. Revise essas tarefas.',
            priority: 'medium'
        });
    }
    ...
    if (project.documents.length === 0) {
        suggestions.push({
            ...
            message: 'Documentos como requisitos, especificações e diagramas ajudam a entender melhor o projeto.',
            priority: 'low'
        });
    }
