...
export function validateJiraFields(task: JiraTask): { valid: boolean; errors: string[] } {
    ...
    if (task.timeTracking) {
        ...
        if (task.timeTracking.originalEstimate && !timePattern.test(task.timeTracking.originalEstimate)) {
            errors.push('Formato de tempo estimado inválido (use formato como "2h 30m")');
        }
        if (task.timeTracking.remainingEstimate && !timePattern.test(task.timeTracking.remainingEstimate)) {
