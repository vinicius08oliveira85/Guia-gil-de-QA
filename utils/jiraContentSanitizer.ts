...
export class JiraContentSanitizer {
   ...
   */
  static sanitize(html: string, config: SanitizationConfig = {}): SanitizedContent {
    if (!html) {
   ...
   */
  private static processJiraImages(html: string, config: SanitizationConfig): string {
    if (!config.jiraUrl) {
      return html;
    }
      ...
      (match, before, src, after) => {
        ...
        if (src.startsWith('http://') || src.startsWith('https://')) {
          if (src.includes('/secure/attachment/') || src.includes(baseUrl)) {
            const hasDataAttr =
              before.includes('data-jira-url=') || after.includes('data-jira-url=');
            ...
            if (
              idMatch &&
              !before.includes('data-attachment-id=') &&
              !after.includes('data-attachment-id=')
            ) {
