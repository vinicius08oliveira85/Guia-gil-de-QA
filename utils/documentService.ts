...
export const createDocumentFromFile = (file: File): Promise<DocumentFile> => {
    return new Promise((resolve, reject) => {
        if (file.size > MAX_DOCUMENT_SIZE) {
            reject(new Error(`Arquivo muito grande. Tamanho máximo: ${MAX_DOCUMENT_SIZE / 1024 / 1024}MB`));
            return;
        }
...
export const convertDocumentFileToProjectDocument = (docFile: DocumentFile): ProjectDocument => {
    if (typeof docFile.content === 'string') {
        ...
        if (docFile.type.startsWith('text/') || docFile.name.endsWith('.txt') || docFile.name.endsWith('.md')) {
            return {
                name: docFile.name,
...
export const getFileIcon = (type: string, name: string): string => {
    ...
    if (type.includes('powerpoint') || type.includes('presentation') || name.endsWith('.ppt') || name.endsWith('.pptx')) return '📽️';
    if (type.includes('zip') || type.includes('rar') || name.endsWith('.zip') || name.endsWith('.rar')) return '📦';
