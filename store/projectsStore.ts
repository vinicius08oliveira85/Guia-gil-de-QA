...
export const useProjectsStore = create<ProjectsState>((set, get) => ({
    ...
    try {
      ...
      if (isSupabaseAvailable()) {
        ...
        if (supabaseProjects.length > 0) {
          logger.info(
            `Projetos carregados do Supabase: ${supabaseProjects.length}`,
            'ProjectsStore'
          );
        } else {
      ...
      if (supabaseProjects.length > 0 && indexedDBProjects.length > 0) {
        ...
        try {
          ...
          if (projectsToBackup.length > 0) {
            logger.debug(
              `Criando backups antes de merge para ${projectsToBackup.length} projetos`,
              'ProjectsStore'
            );
            await Promise.all(
  ...
  syncProjectsFromSupabase: async () => {
    try {
      ...
      logger.info(
        `Projetos sincronizados do Supabase com merge inteligente: ${cleanedProjects.length} (${supabaseProjects.length} do Supabase + ${currentProjects.length} do cache local)`,
        'ProjectsStore'
      );
    } catch (error) {
