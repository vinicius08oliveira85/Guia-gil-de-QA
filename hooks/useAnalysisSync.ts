...
export const useAnalysisSync = ({
  ...
  useEffect(() => {
    ...
    const tasksChanged = previousTasksRef.current !== '' && previousTasksRef.current !== tasksSnapshot;
    const testsChanged = previousTestsRef.current !== '' && previousTestsRef.current !== testsSnapshot;
