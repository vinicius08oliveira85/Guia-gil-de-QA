...
export function useJiraSync(project: Project | null, onUpdateProject: OnUpdateProject) {
  ...
  const performSync = useCallback(
    async (config: JiraConfig, jiraProjectKey: string) => {
      ...
      try {
        ...
        logger.info(
          'Iniciando sincronização com Jira usando projeto mais recente do store',
          'useJiraSync',
          {
        ...
        const missingTaskIds = projectToSync.tasks
        ...
        if (missingTaskIds.length > 0) {
          ...
          if (linkedTaskIds.size > 0) {
            ...
            logger.info(
              `Restauradas ${tasksToRestore.length} tarefas vinculadas que seriam excluídas pela sincronização`,
              'useJiraSync'
            );
          }
        }
        ...
        handleError(
          error instanceof Error ? error : new Error('Erro ao sincronizar com Jira'),
  ...
  const handleSyncJira = useCallback(async () => {
    ...
    if (!config) {
      handleError(
        new Error('Jira não configurado. Configure a conexão com Jira nas Configurações primeiro.'),
        'Sincronizar com Jira'
      );
      return;
    }
    ...
    if (!jiraProjectKey) {
      try {
        ...
        handleError(
          error instanceof Error ? error : new Error('Erro ao buscar projetos do Jira'),
          'Sincronizar com Jira'
        );
      } finally {
        setIsSyncingJira(false);
