...
export const calculateProjectMetrics = (project: Project) => {
  ...
  const tasksWithTestCases = tasks.filter(
    t => t.type === 'Tarefa' && t.testCases && t.testCases.length > 0
  ).length;
  ...
  const testsExecutedAtThreshold =
    totalTestCases > 0 && testsExecutedRatio >= PHASE_COMPLETION_THRESHOLD;
  ...
  const testPassRate =
    executedTestCases > 0 ? Math.round((passedTestCases / executedTestCases) * 100) : 0;

  const bugsBySeverity = openBugs.reduce(
    (acc, bug) => {
  ...
  const initialSeverityCount: Record<BugSeverity, number> = {
    Crítico: 0,
    Alto: 0,
    Médio: 0,
    Baixo: 0,
  ...
  const progressByDay = cumulativeProgress.reduce(
    (acc, val) => {
  ...
  const cumulativeProgressData = Object.entries(progressByDay)
    .map(([date, values]) => ({
      date: Number(date),
  ...
  const documentsWithAnalysis = documents.filter(
    doc => doc.analysis && doc.analysis.length > 0
  ).length;
  ...
  const taskStatusDistribution = [
    {
      ...
      percentage: totalNonBugTasks > 0 ? Math.round((taskStatus.toDo / totalNonBugTasks) * 100) : 0,
    },
    {
      status: 'In Progress',
      count: taskStatus.inProgress,
      percentage:
        totalNonBugTasks > 0 ? Math.round((taskStatus.inProgress / totalNonBugTasks) * 100) : 0,
    },
    {
      status: 'Done',
      count: taskStatus.done,
      percentage: totalNonBugTasks > 0 ? Math.round((taskStatus.done / totalNonBugTasks) * 100) : 0,
    },
    {
      status: 'Blocked',
      count: taskStatus.blocked,
      percentage:
        totalNonBugTasks > 0 ? Math.round((taskStatus.blocked / totalNonBugTasks) * 100) : 0,
    },
  ];
  ...
  const jiraStatusDistribution = Array.from(jiraStatusGrouped.entries())
    .map(([status, tasksList]) => ({
      status,
      count: tasksList.length,
      percentage:
        totalNonBugTasks > 0 ? Math.round((tasksList.length / totalNonBugTasks) * 100) : 0,
      category: categorizeJiraStatus(status),
    }))
    .sort((a, b) => b.count - a.count); // Ordenar por quantidade (maior primeiro)

  // Distribuição por categoria
  const statusCategoryDistribution: Array<{
    ...
    {
      category: 'Concluído',
      count: statusByCategory['Concluído'],
      percentage:
        totalNonBugTasks > 0
          ? Math.round((statusByCategory['Concluído'] / totalNonBugTasks) * 100)
          : 0,
    },
    {
      category: 'Validado',
      count: statusByCategory['Validado'],
      percentage:
        totalNonBugTasks > 0
          ? Math.round((statusByCategory['Validado'] / totalNonBugTasks) * 100)
          : 0,
    },
    {
      category: 'Em Andamento',
      count: statusByCategory['Em Andamento'],
      percentage:
        totalNonBugTasks > 0
          ? Math.round((statusByCategory['Em Andamento'] / totalNonBugTasks) * 100)
          : 0,
    },
    {
      category: 'Pendente',
      count: statusByCategory['Pendente'],
      percentage:
        totalNonBugTasks > 0
          ? Math.round((statusByCategory['Pendente'] / totalNonBugTasks) * 100)
          : 0,
    },
    {
      category: 'Bloqueado',
      count: statusByCategory['Bloqueado'],
      percentage:
        totalNonBugTasks > 0
          ? Math.round((statusByCategory['Bloqueado'] / totalNonBugTasks) * 100)
          : 0,
    },
    {
      category: 'Outros',
      count: statusByCategory['Outros'],
      percentage:
        totalNonBugTasks > 0
          ? Math.round((statusByCategory['Outros'] / totalNonBugTasks) * 100)
          : 0,
    },
  ];
  ...
  const testExecutionDistribution = [
    {
      status: 'Passed',
      count: testExecution.passed,
      percentage:
        totalTestCases > 0 ? Math.round((testExecution.passed / totalTestCases) * 100) : 0,
    },
    {
      status: 'Failed',
      count: testExecution.failed,
      percentage:
        totalTestCases > 0 ? Math.round((testExecution.failed / totalTestCases) * 100) : 0,
    },
    {
      status: 'Not Run',
      count: testExecution.notRun,
      percentage:
        totalTestCases > 0 ? Math.round((testExecution.notRun / totalTestCases) * 100) : 0,
    },
    {
      status: 'Blocked',
      count: testExecution.blocked,
      percentage:
        totalTestCases > 0 ? Math.round((testExecution.blocked / totalTestCases) * 100) : 0,
    },
  ];
  ...
  const averageFailuresPerDay =
    totalTestCases > 0 ? Math.round((failedTestCases / 30) * 10) / 10 : 0;
  ...
  const priorityOrder: (TaskPriority | 'Sem prioridade')[] = [
    'Urgente',
    'Alta',
    'Média',
    'Baixa',
  ...
  const priorityCounts = nonBugTasks.reduce(
    (acc, task) => {
  ...
  const priorityDistribution = priorityOrder
    ...
    .map(priority => ({
      ...
      percentage:
        totalNonBugTasks > 0
          ? Math.round(((priorityCounts[priority] ?? 0) / totalNonBugTasks) * 100)
          : 0,
    }));

  // --- Distribuição por responsável (tarefas não-Bug) ---
  const assigneeCounts = nonBugTasks.reduce(
    (acc, task) => {
  ...
  const complexityCounts = nonBugTasks.reduce(
    (acc, task) => {
  ...
  const complexityDistribution = complexityOrder
    ...
    .map(complexity => ({
      ...
      percentage:
        totalNonBugTasks > 0
          ? Math.round(((complexityCounts[complexity] ?? 0) / totalNonBugTasks) * 100)
          : 0,
    }));
  ...
  return {
    ...
    openVsClosedBugs,
    qualityByModule:
      qualityByModule.length > 0 ? qualityByModule : [{ module: 'Geral', quality: 100 }],
