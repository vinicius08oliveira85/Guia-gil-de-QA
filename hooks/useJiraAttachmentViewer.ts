...
export function useJiraAttachmentViewer() {
  const [viewingJiraAttachment, setViewingJiraAttachment] = useState<JiraAttachmentView | null>(
    null
  );
  const [loadingJiraAttachmentId, setLoadingJiraAttachmentId] = useState<string | null>(null);

  const handleViewJiraAttachment = useCallback(
    async (attachment: { id: string; filename: string; url: string; mimeType?: string }) => {
