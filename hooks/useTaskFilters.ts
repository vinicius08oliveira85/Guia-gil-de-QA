...
export function useTaskFilters(
  ...
  const statusOptions = useMemo(
    () => getStatusFilterOptions(project),
    [project.tasks, jiraStatuses]
  );
    ...
    try {
      ...
      if (raw) {
        ...
        if (data && typeof data === 'object') {
          ...
          if (Array.isArray(data.priorityFilter))
            setPriorityFilter(data.priorityFilter as string[]);
          if (Array.isArray(data.typeFilter)) setTypeFilter(data.typeFilter as string[]);
          if (Array.isArray(data.testStatusFilter))
            setTestStatusFilter(data.testStatusFilter as TaskTestStatus[]);
          if (Array.isArray(data.testCaseExecutionStatusFilter)) {
            const allowed: TestCaseExecutionStatus[] = ['Not Run', 'Passed', 'Failed', 'Blocked'];
            const parsed = (data.testCaseExecutionStatusFilter as string[]).filter(
              (x): x is TestCaseExecutionStatus => (allowed as string[]).includes(x)
            );
          ...
          if (
            ...
            ['id', 'status', 'priority', 'createdAt', 'title'].includes(data.sortBy)
          ) {
            setSortBy(data.sortBy as TaskSortBy);
          }
          if (
            typeof data.groupBy === 'string' &&
            ['none', 'status', 'priority', 'type'].includes(data.groupBy)
          ) {
  ...
  const filteredTasks = useMemo(() => {
    ...
    return tasks.filter(task => {
      ...
      if (priorityFilter.length > 0) {
        if (!priorityFilter.some(name => taskMatchesPriorityName(task, name, project)))
          return false;
      }
      if (typeFilter.length > 0 && !typeFilter.includes(task.type)) return false;
      if (
        testStatusFilter.length > 0 &&
        !testStatusFilter.includes(getEffectiveTestStatus(task, tasks))
      )
        return false;
      if (
        testCaseExecutionStatusFilter.length > 0 &&
        !testCaseExecutionStatusFilter.some(st =>
          (task.testCases || []).some(tc => tc.status === st)
        )
      ) {
        return false;
      }
  ...
  const counts = useMemo(() => {
    ...
    return {
      status: (statusName: string) =>
        allTasks.filter(t => taskMatchesStatusName(t, statusName, project)).length,
      priority: (priorityName: string) =>
        allTasks.filter(t => taskMatchesPriorityName(t, priorityName, project)).length,
      type: (type: string) => allTasks.filter(t => t.type === type).length,
      testStatus: (status: TaskTestStatus) =>
        allTasks.filter(t => getEffectiveTestStatus(t, allTasks) === status).length,
      caseExecution: (status: TestCaseExecutionStatus) =>
        allTasks.filter(t => (t.testCases || []).some(tc => tc.status === status)).length,
      quality: (type: string) => {
        switch (type) {
          case 'with-bdd':
            return allTasks.filter(t => t.bddScenarios?.length).length;
          case 'without-bdd':
            return allTasks.filter(t => !t.bddScenarios?.length).length;
          case 'with-tests':
