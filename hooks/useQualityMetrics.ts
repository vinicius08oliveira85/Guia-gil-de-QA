...
export const calculateQualityMetrics = (project: Project): QualityMetrics => {
  ...
  tasks.forEach(task => {
    ...
    if (task.tags && task.tags.length > 0) {
      const moduleTag = task.tags.find(
        tag => tag.toLowerCase().includes('module:') || tag.toLowerCase().includes('módulo:')
      );
      if (moduleTag) {
  ...
  const topDefectiveModules = Array.from(moduleMap.entries())
    .map(([module, data]) => {
      ...
      const defectDensity =
        totalTasksInModule > 0 ? (openBugsInModule / totalTasksInModule) * 100 : 0;
  ...
  const averageCycleTime =
    cycleTimes.length > 0 ? cycleTimes.reduce((sum, ct) => sum + ct, 0) / cycleTimes.length : 0;

  const sortedCycleTimes = [...cycleTimes].sort((a, b) => a - b);
  const medianCycleTime =
    sortedCycleTimes.length > 0 ? sortedCycleTimes[Math.floor(sortedCycleTimes.length / 2)] : 0;
