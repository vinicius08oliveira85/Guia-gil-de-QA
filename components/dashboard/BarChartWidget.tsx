            ...
            {data.map((item, index) => (
                <div key={item.label} className="flex flex-col items-center flex-1">
                    <div className="w-full h-full flex items-end">
                       {/* Fix: Use rawData for the tooltip to show the actual count instead of the normalized percentage. */}
                       <div className={item.color} style={{ height: `${item.value}%`, width: '100%' }} title={`${item.label}: ${rawData[index].value}`}></div>
                    </div>
