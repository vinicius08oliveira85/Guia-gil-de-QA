          ...
          {payload.map((entry, index) => (
            <div key={index} className="flex items-center gap-2">
              <div
                ...
                aria-hidden="true"
              />
              <span className="text-xs text-base-content/70 flex-1">{entry.name}:</span>
              <span className="font-semibold text-base-content text-sm">{entry.value}%</span>
            </div>
          ))}
        </div>
      </div>
 ...
 * ```
 */
export const QualityMetricsChart = React.memo<QualityMetricsChartProps>(
  ({ project, className }) => {
      ...
      <Card className={className} hoverable>
        <div className="p-6">
          ...
          <div className="h-[300px]">
            {chartData.length === 0 ? (
              <div
                className="flex flex-col items-center justify-center h-full text-base-content/60 text-sm"
                role="status"
                aria-label="Sem dados por fase"
              >
                <p className="font-medium">Sem dados por fase</p>
                <p className="mt-1">
                  Adicione fases e tarefas ao projeto para visualizar as métricas.
                </p>
              </div>
            ) : (
