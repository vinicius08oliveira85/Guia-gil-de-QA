...
export const QualityTrafficLight: React.FC<QualityTrafficLightProps> = ({ project }) => {
    ...
    const statusBorder =
        status === 'Go' ? 'border-success/30' : status === 'Warning' ? 'border-warning/30' : 'border-error/30';
        ...
        <Card className={`relative overflow-hidden border border-base-300 p-6 sm:p-8 ${statusGlow}`}>
            ...
            <div className="relative">
                <div className="flex flex-col items-center justify-center gap-6">
                    {/* Semáforo Visual */}
                    <div className="relative">
                        <div className={`w-24 h-24 sm:w-32 sm:h-32 rounded-full ${statusColor} ${statusBg} border-2 ${statusBorder} flex items-center justify-center transition-all duration-300`}>
                            {status === 'Go' && (
                                <svg className="w-12 h-12 sm:w-16 sm:h-16" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full mt-4">
                        <div className={`rounded-2xl border ${hasCriticalBugs ? 'border-error/30 bg-error/10' : 'border-success/30 bg-success/10'} p-4`}>
                            <div className="flex items-center justify-between">
                                <span className="text-base-content/70 text-sm">Bugs Críticos em Aberto</span>
                                <span className={`text-2xl font-bold ${hasCriticalBugs ? 'text-error' : 'text-success'}`}>
                        ...
                        <div className={`rounded-2xl border ${hasRegressionFail ? 'border-warning/30 bg-warning/10' : 'border-success/30 bg-success/10'} p-4`}>
                            <div className="flex items-center justify-between">
                                <span className="text-base-content/70 text-sm">Regressão Automática</span>
                                <span className={`text-2xl font-bold ${hasRegressionFail ? 'text-warning' : 'text-success'}`}>
