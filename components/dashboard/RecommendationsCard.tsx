...
export const RecommendationsCard: React.FC<RecommendationsCardProps> = React.memo(
  ({ analysis, isLoading = false }) => {
    ...
    const sortedRecommendations = [...analysis.recommendations].sort((a, b) => {
      const aScore =
        (a.impact === 'Alto' ? 3 : a.impact === 'Médio' ? 2 : 1) -
        (a.effort === 'Baixo' ? 0 : a.effort === 'Médio' ? 1 : 2);
      const bScore =
        (b.impact === 'Alto' ? 3 : b.impact === 'Médio' ? 2 : 1) -
            ...
            >
              <div className="space-y-3">
                <div className="flex items-center gap-2 flex-wrap">
                  <Badge
                    variant={getCategoryVariant(rec.category)}
                    size="sm"
                ...
                <h4 className="font-semibold text-base-content text-base leading-relaxed">
                  {rec.title}
                </h4>
