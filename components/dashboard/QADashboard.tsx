 ...
 * ```
 */
export const QADashboard: React.FC<QADashboardProps> = React.memo(
  ({ project, onUpdateProject, onNavigateToTab }) => {
    ...
    const metricsPeriod: 'week' | 'month' =
      dashboardFilters.period === 'month' || dashboardFilters.period === 'quarter'
        ? 'month'
        : 'week';
    ...
    const alerts = useMemo(() => {
      ...
      qualityAlerts.forEach((alertMsg, index) => {
        ...
        alertsList.push({
          ...
          type:
            alertMsg.includes('Crítica') || alertMsg.includes('Elevada') ? 'critical' : 'warning',
