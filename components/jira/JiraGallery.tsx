...
export const JiraGallery: React.FC<JiraGalleryProps> = ({ images, isOpen, initialIndex, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-sm animate-fade-in" onClick={onClose}>
            <button onClick={onClose} className="absolute top-4 right-4 p-2 text-white/70 hover:text-white transition-colors z-50">
                <X size={32} />
            </button>
            ...
            <button onClick={handlePrev} className="absolute left-4 p-3 text-white/50 hover:text-white hover:bg-white/10 rounded-full transition-all z-50">
                <ChevronLeft size={40} />
            </button>

            <div className="relative flex flex-col items-center" onClick={(e) => e.stopPropagation()}>
                <LightboxImage src={currentImage.src} alt={currentImage.alt} mimeType={currentImage.mimeType} />
                <p className="mt-4 text-white/90 text-sm font-medium">{currentImage.filename}</p>
            </div>

            <button onClick={handleNext} className="absolute right-4 p-3 text-white/50 hover:text-white hover:bg-white/10 rounded-full transition-all z-50">
