  ...
  return (
    <div
      ...
      onClick={(e) => {
        ...
        if ((e.target as HTMLElement).closest('.test-checkbox') || 
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-start justify-between gap-sm mb-xs">
              <div className="flex items-center gap-xs flex-1 min-w-0">
                {/* Status dot */}
                <span className="w-2 h-2 rounded-full bg-error flex-shrink-0" aria-label="Teste falhado"></span>
            ...
            {isExpanded && (
              <div className="mt-sm pt-sm border-t border-base-300 space-y-xs">
                {testCase.observedResult && (
                  <div>
                    <p className="text-xs font-semibold text-base-content/70 mb-xs">Resultado Observado:</p>
                ...
                {testCase.expectedResult && (
                  <div>
                    <p className="text-xs font-semibold text-base-content/70 mb-xs">Resultado Esperado:</p>
                    <p className="text-xs text-base-content/80 font-mono bg-base-200 p-xs rounded">
                      {testCase.expectedResult}
                    </p>
                  </div>
            ...
            {(testCase.observedResult || (testCase.steps && testCase.steps.length > 0) || testCase.expectedResult) && (
              <button
                type="button"
                onClick={(e) => {
                  e.stopPropagation();
