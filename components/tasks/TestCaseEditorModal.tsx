...
export const TestCaseEditorModal: React.FC<TestCaseEditorModalProps> = ({
  ...
  const [strategies, setStrategies] = useState<string[]>(
    ensureAtLeastOne(testCase.strategies || [])
  );
  const [executedStrategies, setExecutedStrategies] = useState<string[]>(
    ensureAtLeastOne(normalizeExecutedStrategy(testCase.executedStrategy))
  );
  ...
  const [priority, setPriority] = useState<'Baixa' | 'Média' | 'Alta' | 'Urgente' | ''>(
    testCase.priority || ''
  );
  ...
  const handleListChange =
    (setter: React.Dispatch<React.SetStateAction<string[]>>) => (index: number, value: string) => {
  ...
  const handleListRemove =
    (setter: React.Dispatch<React.SetStateAction<string[]>>, allowRemoval: boolean) =>
    (index: number) => {
      setter(prev => {
        if (!allowRemoval) {
          return prev;
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={`Editar caso de teste: ${testCase.description}`}
      size="xl"
    >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select
              value={priority}
              onChange={e =>
                setPriority(e.target.value as 'Baixa' | 'Média' | 'Alta' | 'Urgente' | '')
              }
              className="select select-bordered w-full bg-base-100 border-base-300 text-base-content focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"
            >
