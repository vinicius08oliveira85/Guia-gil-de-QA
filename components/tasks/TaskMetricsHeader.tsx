    ...
    >
      <div className="flex gap-tasks-panel-tight">
        ...
        <div className="min-w-0 flex-1">
          ...
          {subtitle ? (
            <p className="mt-0.5 line-clamp-2 text-xs leading-snug text-base-content/75">{subtitle}</p>
          ) : null}
        </div>
      </div>
...
export const TaskMetricsHeader: React.FC<TaskMetricsHeaderProps> = React.memo(({ tasks, metricsTasks }) => {
  ...
  return (
    <section className="flex w-full flex-col gap-tasks-panel" aria-labelledby="task-operation-metrics-heading">
      ...
      {criticalOpen > 0 && (
        ...
        >
          ...
          <span>
            <strong className="text-error">{criticalOpen}</strong> crítico(s) em aberto — priorize segurança / LGPD.
          </span>
        </div>
      )}
      ...
      <div className="grid w-full grid-cols-1 gap-tasks-panel-tight sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 xl:gap-tasks-panel">
        ...
        >
          <div className="flex items-baseline justify-between gap-2">
            <span className="text-xl font-bold tabular-nums sm:text-2xl" style={{ color: 'var(--color-primary)' }}>
        ...
        <MicaMetricCard title="Bugs / regressão" subtitle={regressionSubtitle} icon={<Bug className="h-4 w-4" />}>
          ...
          <p className="mt-1.5 text-xs leading-snug text-base-content/75">
            Pós-cerimônia: {regression.regressionBugCount} · Histórias: {regression.storyCount} · Bugs:{' '}
            {regression.bugCount}
          </p>
        </MicaMetricCard>

        <MicaMetricCard title="Débito técnico" subtitle="Manutenção (flag ou título/tags)" icon={<Wrench className="h-4 w-4" />}>
          <p className="text-xl font-bold tabular-nums sm:text-2xl" style={{ color: 'var(--color-primary)' }}>
            {debtCount}
          </p>
        ...
        <MicaMetricCard title="Falhas por peso" subtitle="Bugs abertos (pontos ou severidade)" icon={<BarChart3 className="h-4 w-4" />}>
        ...
        >
          ...
          <p className="mt-1 text-xs leading-snug text-base-content/75">
            {criticalOpen === 0 ? 'Nada sensível pendente neste escopo.' : 'Priorize regressão e testes de segurança.'}
          </p>
        </MicaMetricCard>
      </div>
    </section>
