...
export const TasksView: React.FC<{
  ...
  const filteredTasks = useMemo(() => {
    return project.tasks.filter(task => {
      ...
      if (
        ...
        !statusFilter.some(name => taskMatchesStatusName(task, name, project))
      )
        return false;
      ...
      if (
        ...
        !priorityFilter.some(name => taskMatchesPriorityName(task, name, project))
      )
        return false;
  ...
  const counts = useMemo(() => {
    ...
    allTasks.forEach(task => {
      ...
      if (hasBDD) results.quality['with-bdd']++;
      else results.quality['without-bdd']++;
      if (hasTests) {
    ...
    project,
    project.tasks,
    project.settings?.jiraStatuses,
    project.settings?.jiraPriorities,
    taskEffectiveStatuses,
  ]);
  ...
  const withTimeout = <T,>(promise: Promise<T>, timeoutMs: number = 60000): Promise<T> => {
    return Promise.race([
      ...
      new Promise<T>((_, reject) =>
        setTimeout(
          () => reject(new Error(`Timeout: operação excedeu ${timeoutMs / 1000} segundos`)),
          timeoutMs
        )
      ),
    ]);
  ...
  const handleDeleteBddScenario = useCallback(
  ...
  const handleDeleteTestCase = useCallback(
    (taskId: string, testCaseId: string) => {
      ...
      const confirmed = window.confirm(
        'Deseja realmente excluir este caso de teste? Esta ação não pode ser desfeita.'
      );
      if (!confirmed) return;
  ...
  const renderTaskTree = useCallback(
    (tasks: TaskWithChildren[], level: number, startIndex: number = 0): React.ReactElement[] => {
        ...
        <div className="flex flex-col gap-6 mb-8">
          ...
          <div className="mb-6 lg:mb-8">
            ...
            <p className="text-xs text-base-content/70 mt-2">
              Filtre tarefas e casos instantaneamente sem precisar abrir o painel completo de
              filtros.
            </p>
          </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              {(generatingTestsTaskId || generatingBddTaskId) && (
                <div className="p-4 bg-primary/10 border border-primary/40 rounded-lg text-sm text-base-content flex items-center gap-2">
                  ...
                  {generatingTestsTaskId && (
                    <span>Gerando casos de teste para {generatingTestsTaskId}...</span>
                  )}
                  {generatingBddTaskId && (
                    <span>Gerando cenários BDD para {generatingBddTaskId}...</span>
                  )}
                </div>
              )}
            </div>
            ...
            >
              <div className="space-y-4">
                <p className="text-base-content/70 text-sm">
                  Selecione os projetos para onde deseja vincular as {selectedTasks.size} tarefa(s)
                  selecionada(s). As tarefas serão adicionadas aos projetos selecionados e manterão
                  sincronia de conteúdo.
                </p>
              ...
              if (t.id === taskId) {
                ...
                return {
                  ...
                  children: children.map(
                    child => findTaskWithChildren(tasks, child.id) || { ...child, children: [] }
                  ),
                } as TaskWithChildren;
              }
            }
