...
export const JiraTaskItem: React.FC<{
  ...
  ({
    ...
    const [viewingJiraAttachment, setViewingJiraAttachment] = useState<{
      id: string;
      filename: string;
      url: string;
      mimeType: string;
      ...
      const loadTestStatus = async () => {
        ...
        try {
          ...
          if (status !== null) {
            ...
            const calculatedStatus = calculateTaskTestStatus(
              task,
              project?.tasks || [],
              tasksByParent
            );
    ...
    const testStatusConfig = useMemo(() => {
      const status =
        taskTestStatus || calculateTaskTestStatus(task, project?.tasks || [], tasksByParent);
    ...
    const handleViewJiraAttachment = async (attachment: {
      id: string;
      filename: string;
      url: string;
      mimeType?: string;
          ...
          <div className="flex items-center gap-2 p-2.5 rounded-xl bg-base-200 border border-base-300 mt-1.5">
            ...
            {taskTestStatus === 'testando' && (
              <button
                ...
                className="btn btn-sm btn-success text-white shadow-sm rounded-xl"
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-2">
            ...
            {nextStep && (
              <div className="p-2.5 bg-orange-50 dark:bg-orange-950/20 border border-brand-orange/30 rounded-xl">
                <p className="text-[0.65rem] uppercase text-brand-orange tracking-wide">
                  Próximo passo
                </p>
                      ...
                      <div className="p-2.5 bg-base-100 border border-base-300 rounded-xl">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {task.components.map(comp => (
                            <span
                              key={comp.id}
                              className="text-xs px-2 py-1 bg-blue-500/20 text-blue-700 dark:text-blue-400 rounded"
                            >
                              {comp.name}
                            </span>
                          ))}
                      ...
                      <div className="p-2.5 bg-base-100 border border-base-300 rounded-xl">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {task.fixVersions.map(version => (
                            <span
                              key={version.id}
                              className="text-xs px-2 py-1 bg-green-500/20 text-green-700 dark:text-green-400 rounded"
                            >
                              {version.name}
                            </span>
                          ))}
    ...
    const renderTestsSection = () => {
      ...
      return (
        <div className="space-y-3">
          {/* Sub-abas de Testes */}
          <div
            className="flex flex-wrap gap-1.5 p-1 bg-base-200 rounded-xl w-fit"
            role="tablist"
                ...
                <div className="space-y-2">
                  {task.testCases.map(tc => (
                    <TestCaseItem
                      ...
                      onDelete={
                        onDeleteTestCase ? () => onDeleteTestCase(task.id, tc.id) : undefined
                      }
                    />
                  ))}
                </div>
            ...
            <div className="flex items-center gap-2 flex-1 min-w-0 order-1 w-full sm:w-auto">
              {onToggleSelect && (
                <input
                  ...
                  aria-label={
                    isSelected ? `Tarefa ${task.id} selecionada` : `Selecionar tarefa ${task.id}`
                  }
                />
              )}
              {hasChildren ? (
                ...
                >
                  <ChevronDown
                    className={`w-4 h-4 transition-transform flex-shrink-0 ${isChildrenOpen ? 'rotate-180' : ''}`}
                    aria-hidden="true"
                  />
              ...
              <span
                className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded tracking-wider shrink-0 ${typeBadgeClass}`}
              >
                {task.type}
              </span>
              <span className="text-xs font-medium text-base-content/60 shrink-0">{task.id}</span>
              <span className="text-sm font-semibold text-base-content truncate min-w-0 flex-1">
                {task.title}
              </span>
            </div>

            <div
              className="flex items-center gap-1.5 sm:gap-2 flex-wrap sm:flex-nowrap flex-shrink-0 w-full sm:w-auto sm:ml-auto order-2"
            ...
            >
              {(task.type === 'Tarefa' || task.type === 'Bug') && (
                ...
                >
                  ...
                  <div
                    className="w-4 h-4 sm:w-5 sm:h-5 rounded-full bg-amber-500 flex items-center justify-center text-[9px] sm:text-[10px] text-white font-bold"
                    title="Pendentes"
              ...
              {['tarefa', 'bug', 'task'].includes(taskTypeNorm) && onGenerateAll && (
                <button
                  ...
                  aria-label={
                    isGenerating || isGeneratingAll ? 'Gerando tudo' : 'Gerar Tudo (BDD e Testes)'
                  }
                >
