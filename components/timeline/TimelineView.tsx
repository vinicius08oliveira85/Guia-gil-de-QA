...
const Checkbox: React.FC<{ checked: boolean }> = ({ checked }) => (
    <div className={`w-4 h-4 rounded border-2 ${checked ? 'bg-teal-500 border-teal-500' : 'border-gray-500'} flex items-center justify-center`}>
                ...
                <table className="w-full min-w-[1000px] text-left text-sm">
                    ...
                    <tbody className="divide-y divide-gray-700">
                        {timelineData.map(row => {
                            ...
                            return (
                                <tr key={row.phase} className={`${isCurrent ? 'bg-teal-500/10' : ''} ${isDone ? 'opacity-60' : ''}`}>
                                    <td className="p-3 font-semibold text-teal-400 align-top">{row.phase}</td>
                                    <td className="p-3 text-gray-300 align-top">{row.duration}</td>
                                    <td className="p-3 text-gray-300 align-top">{row.dependencies}</td>
                                    <td className="p-3 text-gray-300 align-top">{row.exitCriteria}</td>
                                    <td className="p-3 text-gray-300 align-top">{row.milestone}</td>
