        ...
        <section className="space-y-6">
          <header className="space-y-3">
            <div>
              ...
              <h3 className="text-2xl font-semibold text-text-primary">{currentSchema.label}</h3>
            </div>
            <p className="text-text-secondary leading-relaxed">{currentSchema.description}</p>
            <div className="flex flex-wrap gap-2 text-sm text-text-secondary">
              {currentSchema.totalHint && <Badge size="sm">{currentSchema.totalHint}</Badge>}
              {currentSchema.primaryKey && <Badge size="sm">PK: {currentSchema.primaryKey}</Badge>}
              ...
              <div className="bg-surface-hover border border-surface-border rounded-2xl p-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {currentSchema.statuses.map(status => (
                    <Badge key={status} variant="info" size="sm" className="capitalize">
                      {status}
                    </Badge>
                  ))}
                </div>
                  ...
                  {filteredFields.map(field => (
                    <tr key={field.name} className="border-t border-surface-border">
                      ...
                      <td className="px-4 py-3 text-text-tertiary">
                        {field.example ? (
                          <code className="text-xs bg-surface-hover px-2 py-1 rounded-lg">
                            {field.example}
                          </code>
                        ) : (
                          '—'
