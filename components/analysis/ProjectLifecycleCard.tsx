...
const phaseNamesInOrder: PhaseName[] = [
  'Request',
  'Analysis',
  'Design',
  'Analysis and Code',
...
export const ProjectLifecycleCard: React.FC<{ project: Project }> = ({ project }) => {
  ...
  const selectedPhaseData = selectedPhase
    ? project.phases.find(p => p.name === selectedPhase)
    : null;
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap justify-between items-center gap-x-4 gap-y-1">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <div
                        className="tooltip-container flex-shrink-0"
                    ...
                    <span
                      className={`text-xs font-semibold py-1 px-2 rounded-full ${bg} ${text} flex-shrink-0`}
                    >
                      {phase.status}
                    </span>
                  </div>
        ...
        {selectedPhaseData && (
          <div className="space-y-4">
            <div>
              ...
              <span
                className={`text-sm font-semibold py-1 px-3 rounded-full ${getStatusStyles(selectedPhaseData.status).bg} ${getStatusStyles(selectedPhaseData.status).text}`}
              >
            ...
            <div>
              ...
              <p className="text-sm text-text-secondary">
                {phaseExplanations[selectedPhaseData.name]}
              </p>
            </div>
