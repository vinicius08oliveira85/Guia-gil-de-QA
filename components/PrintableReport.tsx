...
export const PrintableReport: React.FC<{ project: Project; metrics: ReportMetrics }> = ({
  project,
  metrics,
}) => {
  ...
  return (
    <div
      id="printable-report"
      style={{ fontFamily: 'sans-serif', color: '#000', backgroundColor: '#fff', padding: '20px' }}
    >
      {/* Header */}
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
          <tbody>
            <tr style={{ borderBottom: '1px solid #f0f0f0' }}>
              ...
              <td style={{ padding: '10px 8px', fontSize: '18px', fontWeight: 'bold' }}>
                {metrics.testCoverage}%
              </td>
              <td style={{ padding: '10px 8px', color: '#555' }}>
                ({metrics.tasksWithTestCases} de {metrics.totalTasks} tarefas cobertas)
              </td>
            </tr>
            <tr style={{ borderBottom: '1px solid #f0f0f0' }}>
              ...
              <td style={{ padding: '10px 8px', color: '#555' }}>
                ({metrics.passedTestCases} de {metrics.executedTestCases} testes aprovados)
              </td>
            </tr>
            <tr style={{ borderBottom: '1px solid #f0f0f0' }}>
              <td style={{ padding: '10px 8px', fontWeight: 'bold' }}>
                Relação Automação vs. Manuais:
              </td>
              <td style={{ padding: '10px 8px', fontSize: '18px', fontWeight: 'bold' }}>
                {metrics.automationRatio}%
              </td>
              <td style={{ padding: '10px 8px', color: '#555' }}>
                ({metrics.automatedTestCases} de {metrics.totalTestCases} testes automatizados)
              </td>
            </tr>
          </tbody>
        </table>
          ...
          <tbody>
            {project.phases.map(phase => (
              <tr key={phase.name}>
                ...
                <td style={{ padding: '8px', border: '1px solid #ddd' }}>
                  {phase.summary || 'Análise da IA pendente.'}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <div style={{ pageBreakBefore: 'always', paddingTop: '20px' }}>
        ...
        {tasksToReport.length > 0 ? (
              ...
              <div style={{ padding: '10px' }}>
                {task.testCases && task.testCases.length > 0 ? (
                  ...
                  <p style={{ color: '#777', fontStyle: 'italic', margin: 0 }}>
                    Nenhum caso de teste associado.
                  </p>
                )}
              </div>
            </div>
          ...
          <p style={{ color: '#777' }}>
            Nenhuma tarefa (História, Tarefa, Epic) encontrada neste projeto.
          </p>
        )}
      </div>
    </div>
