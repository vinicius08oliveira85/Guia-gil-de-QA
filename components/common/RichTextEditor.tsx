...
export const RichTextEditor: React.FC<RichTextEditorProps> = ({
  ...
  const applyFormat = (
    fn: (v: string, s: number, e: number) => { newValue: string; newCursor: number }
  ) => {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      handleError(
        error instanceof Error ? error : new Error('Erro ao fazer upload'),
        'Upload de arquivo'
      );
    } finally {
      setUploading(false);
  ...
  return (
    <div className="space-y-2">
      ...
      <p className="text-xs text-text-secondary">
        💡 Use a barra acima para negrito, listas com marcadores ou numeradas; o último botão insere
        imagens ou arquivos.
      </p>
    </div>
  );
