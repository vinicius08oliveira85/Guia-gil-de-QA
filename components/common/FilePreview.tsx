...
import { FileText, Image as ImageIcon, File, FileCode, FileSpreadsheet, FileType, Archive } from 'lucide-react';
...
export const FilePreview: React.FC<FilePreviewProps> = ({
    ...
    const { objectUrl, thumbnailUrl, loading, loadingThumbnail, error, mediaInfo } = useJiraMedia(attachmentId, filename, size, {
        useProxy: true,
            ...
            >
                {loading || loadingThumbnail ? (
                    ...
                    <div className="flex flex-col items-center justify-center w-full h-full bg-base-200 text-base-content/70 p-2 text-center">
                        ...
                        <span className="text-xs mt-0.5 text-error/80">{typeof error === 'string' ? error : 'Erro ao carregar'}</span>
 ...
 */
function getIconForMediaType(mediaType: MediaType, size: number = 32): React.ComponentType<{ className?: string }> {
    switch (mediaType) {
        case 'pdf':
        ...
        case 'spreadsheet':
            return (props) => <FileSpreadsheet size={size} className={`text-green-500 ${props.className || ''}`} />;
        case 'code':
            return (props) => <FileCode size={size} className={`text-yellow-500 ${props.className || ''}`} />;
        case 'archive':
            return (props) => <Archive size={size} className={`text-purple-500 ${props.className || ''}`} />;
        default:
            return (props) => <File size={size} className={`text-base-content/50 ${props.className || ''}`} />;
    }
}
 ...
 */
export const PDFPreview: React.FC<Omit<FilePreviewProps, 'mediaType'> & { onView?: () => void }> = ({
    attachmentId,
    filename,
    size,
    url,
