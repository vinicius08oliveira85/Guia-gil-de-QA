...
const MetricRing: React.FC<MetricRingProps> = ({
  ...
  return (
    ...
    >
      ...
      <span className="text-xs text-base-content/50 leading-tight">{metric.trend}%</span>
    </div>
  );
};
...
export const ProjectActivityCard: React.FC<ProjectActivityCardProps> = ({
  ...
  const getPriorityBgColor = (priority?: TaskPriority): string => {
    switch (priority) {
      case 'Urgente':
        return 'bg-error/10 hover:bg-error/15 border-error/20';
      case 'Alta':
        return 'bg-warning/10 hover:bg-warning/15 border-warning/20';
      case 'Média':
        return 'bg-info/10 hover:bg-info/15 border-info/20';
      case 'Baixa':
  ...
  const metrics = useMemo(() => {
    ...
    const testExecutionPercent =
      projectMetrics.totalTestCases > 0
    ...
    const bugsCompletedPercent = totalBugs > 0 ? Math.round((completedBugs / totalBugs) * 100) : 0;
    ...
    >
      ...
      <div className="flex items-start gap-2 sm:gap-3 mb-4 sm:mb-6">
        ...
        <div className="flex-1 min-w-0">
          ...
          <h3
            className="text-base sm:text-lg font-semibold text-base-content line-clamp-3 min-h-[3rem]"
            title={project.name}
          >
            {project.name}
          </h3>
                ...
                >
                  <CheckCircle2
                    className={cn(
                      ...
                      goal.isCompleted ? 'text-success' : 'text-base-content/30'
                    )}
                  />
                  <span
                    className={cn(
