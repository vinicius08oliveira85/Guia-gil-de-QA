...
export const SupabaseManager: React.FC<SupabaseManagerProps> = ({ project, onProjectUpdated }) => {
  ...
  const handleSaveToSupabase = async () => {
    if (!supabaseAvailable) {
      handleWarning(
        'Supabase não está configurado. Configure o proxy (VITE_SUPABASE_PROXY_URL) e as variáveis do servidor no Vercel.'
      );
      return;
    }
  ...
  const handleDeleteFromSupabase = async () => {
    ...
    if (
      !confirm(
        `Tem certeza que deseja excluir o projeto "${project.name}" do Supabase?\n\nEsta ação não pode ser desfeita.`
      )
    ) {
      return;
    }
      ...
      <Card className="border-yellow-500/30 bg-yellow-500/10">
        <div className="p-4">
          ...
          <p className="text-sm text-text-secondary mb-4">
            ...
            <code className="bg-black/30 px-1 rounded">SUPABASE_URL</code>,{' '}
            <code className="bg-black/30 px-1 rounded">SUPABASE_SERVICE_ROLE_KEY</code> e{' '}
            <code className="bg-black/30 px-1 rounded">VITE_SUPABASE_PROXY_URL</code> no Vercel para
            habilitar o armazenamento na nuvem.
          </p>
          <p className="text-xs text-text-secondary">
            📊 Projeto atual: <strong>{project.tasks.length} tarefas</strong> - Armazenado
      ...
      <div className="p-4">
        ...
        <div className="mb-4">
          ...
          <div className="text-xs text-text-secondary space-y-1">
            <p>
              📊 <strong>{project.tasks.length}</strong> tarefas no projeto
            </p>
