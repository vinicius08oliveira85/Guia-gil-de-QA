      ...
      <div className="flex min-w-0 flex-col items-center gap-1 text-center">
        <div className="relative h-10 w-10 flex-shrink-0 rounded-full bg-base-200/50 p-0.5 shadow-inner ring-1 ring-base-content/[0.06] transition-[box-shadow,transform] duration-200 group-hover:ring-base-content/10 motion-reduce:transition-none">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="text-[10px] font-bold tabular-nums text-base-content">
              {Math.round(clamped)}%
            </span>
          </div>
        </div>
        <span className="text-[11px] font-semibold leading-tight tracking-wide text-base-content/78">
          {label}
        </span>
      </div>
    );
  ...
  return (
    <svg
      className={cn(
        ...
        subtle
          ? '-right-3 -top-3 h-24 w-24 sm:h-28 sm:w-28'
          : '-right-6 -top-6 h-44 w-44 sm:h-52 sm:w-52'
      )}
      viewBox="0 0 200 200"
      fill="none"
      aria-hidden
...
export const ProjectCard = React.memo<ProjectCardProps>(
  ({ project, onSelect, onTaskClick, icon: Icon = Layout, className, layout = 'row' }) => {
