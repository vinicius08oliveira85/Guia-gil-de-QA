  ...
  return (
    <div className="space-y-4">
      ...
      {timeline.length > 0 ? (
          ...
          <div className="space-y-4">
            {timeline.map((event, index) => (
              <div key={event.id || index} className="relative flex items-start gap-4">
                <div
                  className={`relative z-10 w-8 h-8 rounded-full ${getEventColor(event.action)} flex items-center justify-center text-white text-sm`}
                >
                ...
                <div className="flex-1 pb-4">
                  <div className="p-3 bg-surface border border-surface-border rounded-lg">
                    ...
                    {event.task && (
                      <div className="text-sm text-text-secondary mb-2">{event.task.title}</div>
                    )}
        ...
        <div className="text-center py-8 text-text-secondary">Nenhum evento registrado</div>
      )}
    </div>
  );
};
