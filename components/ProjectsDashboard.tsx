...
export const ProjectsDashboard: React.FC<{
  ...
  const { importProject, supabaseLoadFailed, supabaseLoadError, loadProjects, isLoading } =
    useProjectsStore();
  ...
  const loadJiraProjects = async (useCache: boolean = true) => {
    try {
      ...
      logger.info('Carregando projetos do Jira', 'ProjectsDashboard', {
        url: jiraConfig.url,
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="space-y-0.5">
                        ...
                        <p className="text-sm text-base-content/70">
                          Importe um projeto existente do Jira com todas as tarefas e casos de
                          teste.
                        </p>
                      </div>
                    </div>
        ...
        <div className="mt-8">
          {filteredProjects.length > 0 ? (
            ...
            <>
              {supabaseLoadFailed && (
                <div
                  className="mb-4 p-3 rounded-lg bg-warning/10 text-warning-content border border-warning/30 text-sm flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"
                  role="alert"
                >
                  <div className="flex flex-col gap-1 min-w-0">
                    <span>
                      Sincronização com a nuvem indisponível no momento. Se você já tinha projetos,
