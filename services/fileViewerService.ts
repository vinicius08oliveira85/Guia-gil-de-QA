...
export const detectFileType = (fileName: string, mimeType?: string): FileViewerType => {
  ...
  if (mimeType?.startsWith('image/') || lowerName.match(/\.(jpg|jpeg|png|gif|bmp|webp|svg)$/)) {
    return 'image';
  }
...
export const viewFileInNewTab = (
  ...
  try {
    ...
    throw new Error(
      `Erro ao visualizar arquivo: ${error instanceof Error ? error.message : 'Erro desconhecido'}`
    );
  }
};
...
export const getFileViewerURL = (
  ...
  try {
    ...
    throw new Error(
      `Erro ao criar URL: ${error instanceof Error ? error.message : 'Erro desconhecido'}`
    );
  }
};
