...
if (supabaseUrl && supabaseAnonKey) {
  ...
  if (isProduction()) {
    logger.warn(
      'Supabase não configurado. Configure VITE_SUPABASE_URL e VITE_SUPABASE_ANON_KEY para salvamento direto, ou VITE_SUPABASE_PROXY_URL para usar proxy. Usando apenas armazenamento local (IndexedDB).',
      'supabaseService'
    );
  } else {
    logger.warn(
      'Supabase não configurado. Usando apenas armazenamento local (IndexedDB).',
      'supabaseService'
    );
  }
}
    ...
    try {
      ...
      if (supabase) {
        try {
          ...
          if (isNetworkError(error)) {
            logger.warn(
              'Erro de rede ao salvar via SDK Supabase. Salvando apenas localmente.',
              'supabaseService',
              error
            );
      ...
      if (supabaseProxyUrl) {
        try {
          ...
          if (isNetworkError(error)) {
            logger.warn(
              'Erro de rede ao salvar via proxy Supabase. Salvando apenas localmente.',
              'supabaseService',
              error
            );
            throw error;
  ...
  if (supabaseProxyUrl) {
    ...
    try {
      ...
      if (isCorsError(error)) {
        ...
        logger.warn(
          'Timeout ao carregar projetos do Supabase (proxy). Usando cache local',
          'supabaseService',
          error
        );
      ...
      if (isSchemaCacheOrPaused || errorMessage.includes('500') || isTimeout) {
        try {
          if (isTimeout) {
            logger.debug(
              'Retry em 2s após timeout ao carregar projetos do Supabase',
              'supabaseService'
            );
          }
