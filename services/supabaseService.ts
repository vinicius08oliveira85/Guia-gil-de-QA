...
const saveDebounceMs = 2000; // Reduz picos de salvamento durante geração por IA (menos 503/timeouts)
...
if (supabaseUrl && supabaseAnonKey) {
  ...
  if (!isProduction()) {
    void Promise.race([
      ...
      new Promise<never>((_, reject) =>
        setTimeout(() => reject(new Error('Timeout ao autenticar anonimamente')), 3000)
      ),
    ])
      .then(result => {
        if (result.error) {
          logger.debug(
            'Erro ao autenticar anonimamente no Supabase',
            'supabaseService',
            result.error
          );
          return;
        ...
        logger.info(
          'Supabase configurado via SDK (salvamento direto habilitado)',
          'supabaseService'
        );
      })
      .catch(error => {
  ...
  if (isProduction()) {
    logger.warn(
      'Supabase não configurado. No frontend use variáveis com prefixo VITE_: VITE_SUPABASE_PROXY_URL (recomendado) ou VITE_SUPABASE_URL e VITE_SUPABASE_ANON_KEY. Usando apenas armazenamento local (IndexedDB).',
      'supabaseService'
    );
  } else {
    logger.warn(
      'Supabase não configurado. Use VITE_SUPABASE_PROXY_URL ou VITE_SUPABASE_URL + VITE_SUPABASE_ANON_KEY no .env. Usando apenas armazenamento local (IndexedDB).',
      'supabaseService'
    );
  }
}
  ...
  const fetchPromise = fetch(url, {
    ...
    .catch(error => {
      ...
      if (isCorsError(error)) {
        throw new Error(
          'CORS: Não é possível acessar Supabase diretamente. Configure VITE_SUPABASE_PROXY_URL.'
        );
      }
      throw error;
    });
...
const saveThroughProxy = async (project: Project, retryCount: number = 0): Promise<void> => {
  ...
  if (payloadSizeMB > 1 && typeof CompressionStream !== 'undefined') {
    try {
      ...
      logger.debug(
        `Payload comprimido: ${payloadSizeMB.toFixed(2)}MB -> ${compressedSizeMB.toFixed(2)}MB`,
        'supabaseService'
      );
    } catch (error) {
  ...
  try {
    ...
    if (isNetworkError(error) && retryCount < maxRetries) {
      ...
      if (retryCount === 0) {
        logger.debug(
          `Erro de rede ao salvar via proxy. Tentando novamente em ${delay}ms...`,
          'supabaseService'
        );
      }
...
const saveThroughSdk = async (project: Project, retryCount: number = 0): Promise<void> => {
  ...
  const savePromise = supabase.from('projects').upsert(
    {
      id: project.id,
      user_id: userId,
      name: project.name,
  ...
  if (error) {
    ...
    if (isNetworkError(error) && retryCount < maxRetries) {
      ...
      if (retryCount === 0) {
        logger.debug(
          `Erro de rede ao salvar via SDK. Tentando novamente em ${delay}ms...`,
          'supabaseService'
        );
      }
      await new Promise(resolve => setTimeout(resolve, delay));
...
export const saveProjectToSupabase = async (project: Project): Promise<void> => {
  if (forceLocalOnly) {
    logger.debug(
      'VITE_LOCAL_ONLY=true: salvamento remoto ignorado (local-first)',
      'supabaseService',
  ...
  if (isSupabaseRemotePaused()) {
    logger.debug(
      'Supabase em pausa após falhas transitórias: salvamento remoto adiado',
      'supabaseService',
      {
        projectId: project.id,
      }
  ...
  if (existingSave) {
    ...
    try {
      ...
      if (latestProject && latestProject !== project) {
        ...
        logger.debug(
          `Versão mais recente do projeto "${project.name}" encontrada na fila, salvando novamente`,
          'supabaseService'
        );
  ...
  const savePromise = (async () => {
    try {
      ...
      logger.debug(
        `Iniciando salvamento do projeto "${project.name}" (${projectId})`,
        'supabaseService',
        {
          projectId,
      ...
      if (useProxyFirst && supabaseProxyUrl) {
        try {
          logger.debug(
            `Salvando projeto "${project.name}" via proxy (produção)`,
            'supabaseService',
            {
              projectId,
              projectName: project.name,
          ...
          if (isPayloadTooLargeError(error)) {
            if (supabase) {
              try {
                ...
                logger.info(
                  `Projeto "${project.name}" salvo com sucesso via SDK (proxy retornou 413)`,
                  'supabaseService',
                  {
                    projectId,
                    projectName: project.name,
                ...
                logger.warn(
                  'Erro ao salvar via SDK após 413 do proxy',
                  'supabaseService',
                  sdkError
                );
              }
      ...
      if (supabase) {
        try {
          ...
          if (isNetworkError(error)) {
            ...
            logger.debug(
              'Erro de rede ao salvar via SDK Supabase. Tentando proxy como fallback.',
              'supabaseService',
              error
            );
          }
          if (isForbiddenError(error)) {
            ...
            logger.debug(
              'Erro ao salvar via SDK Supabase, tentando proxy como fallback',
              'supabaseService',
              error
            );
          }
      ...
      if (supabaseProxyUrl) {
        try {
          ...
          if (isPayloadTooLargeError(error)) {
            ...
            logger.debug(
              'Erro 413: Payload muito grande para salvar via proxy. Tentando SDK direto...',
              'supabaseService',
              error
            );
            try {
              ...
              logger.warn(
                'Erro ao salvar via SDK após falha do proxy',
                'supabaseService',
                sdkError
              );
              throw new Error(
      ...
      if (!supabase && !supabaseProxyUrl) {
        if (isProduction()) {
          logger.debug(
            'Supabase não configurado. Projeto salvo apenas localmente.',
            'supabaseService'
          );
        } else {
          logger.debug(
...
export const loadProjectsFromSupabase = async (): Promise<LoadProjectsResult> => {
  ...
  if (supabaseProxyUrl) {
    ...
    const attempt = async (): Promise<LoadProjectsResult> => {
      const userId = await getUserId();
      const response = await callSupabaseProxy<{ projects?: Project[] }>(
        'GET',
        {
          query: { userId },
        },
      ...
      if (projects.length === 0) {
        ...
        logger.info(
          `${projects.length} projetos carregados do Supabase via proxy`,
          'supabaseService'
        );
      }
      return { projects, loadFailed: false };
    ...
    for (let attemptIndex = 0; attemptIndex < maxLoadAttempts; attemptIndex++) {
      try {
        ...
        const errorMessage =
          safeUnknownToMessageText(error) || 'Erro desconhecido ao carregar projetos';
        ...
        if (isCorsError(error)) {
          logger.error(
            'Erro CORS ao carregar do Supabase. Configure VITE_SUPABASE_PROXY_URL',
            'supabaseService',
            error
          );
          return {
            projects: [],
            loadFailed: true,
            errorMessage: normalizeLoadErrorForUser(errorMessage),
          };
        }
        ...
        logger.debug(
          `Aguardando ${Math.round(delayMs / 1000)}s antes de retentar carregar projetos (Supabase cold start/5xx)`,
          'supabaseService'
        );
  ...
  if (!supabase || isProduction()) {
    if (isProduction()) {
      logger.warn(
        'Supabase não disponível em produção sem proxy. Configure VITE_SUPABASE_PROXY_URL.',
        'supabaseService'
      );
    }
    return { projects: [], loadFailed: false };
  ...
  try {
    ...
    if (error) {
      const errMsg =
        safeUnknownToMessageText(error?.message ?? error) || 'Erro ao consultar projetos';
      if (isCorsError(error)) {
        logger.error(
          'Erro CORS ao carregar via SDK. Use proxy em produção',
          'supabaseService',
          error
        );
      } else {
      ...
      return {
        projects: [],
        loadFailed: true,
        errorMessage: getSupabaseLoadFailureUserMessage(errMsg),
      };
    }
...
export const deleteProjectFromSupabase = async (projectId: string): Promise<void> => {
  ...
  if (supabaseProxyUrl) {
    try {
      ...
      if (isCorsError(error)) {
        logger.error(
          'Erro CORS ao deletar do Supabase. Configure VITE_SUPABASE_PROXY_URL',
          'supabaseService',
          error
        );
      } else {
  ...
  if (!supabase || isProduction()) {
    if (isProduction()) {
      logger.warn(
        'Supabase não configurado em produção. Configure VITE_SUPABASE_PROXY_URL. Projeto deletado apenas localmente',
        'supabaseService'
      );
    } else {
      logger.warn(
        'Supabase não configurado, projeto deletado apenas localmente',
        'supabaseService'
      );
    }
    return;
  ...
  try {
    ...
    const timeoutPromise = createTimeoutPromise<{ error: { message: string } }>(
      requestTimeoutMs,
      timeoutErrorMessage
    );
    ...
    if (error) {
      if (isCorsError(error)) {
        logger.error(
          'Erro CORS ao deletar via SDK. Use proxy em produção',
          'supabaseService',
          error
        );
      } else {
        logger.warn(
          'Erro ao deletar projeto do Supabase (deletado apenas localmente)',
          'supabaseService',
          error
        );
      }
    ...
    if (isCorsError(error)) {
      ...
      logger.warn(
        'Erro ao deletar do Supabase (deletado apenas localmente)',
        'supabaseService',
        error
      );
    }
...
export const diagnoseSupabaseConfig = (): {
  ...
  if (hasProxy && hasSDK) {
    ...
    details =
      'Supabase não configurado. Usando apenas armazenamento local (IndexedDB). Configure no .env: VITE_SUPABASE_PROXY_URL (recomendado) ou VITE_SUPABASE_URL e VITE_SUPABASE_ANON_KEY (prefixo VITE_ é obrigatório no frontend).';
  }
...
export const loadTestStatusesByJiraKeys = async (
  ...
  try {
    ...
    if (projects.length === 0) {
      logger.debug(
        'Nenhum projeto encontrado no Supabase para buscar status de testes',
        'supabaseService'
      );
      return result;
    }
        ...
        if (task.id && isValidJiraKey(task.id) && keysSet.has(task.id)) {
          ...
          if (!result.has(task.id) || (task.testCases && task.testCases.length > 0)) {
            ...
            if (testCases.length > 0) {
              ...
              logger.debug(
                `Status de testes encontrados para ${task.id}: ${testCases.length} casos (${testCasesWithStatus} com status)`,
                'supabaseService'
              );
            }
          }
    ...
    logger.warn(
      'Erro ao buscar status de testes do Supabase, continuando sem preservar status',
      'supabaseService',
      error
    );
