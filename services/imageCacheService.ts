...
export class ImageCacheService {
    ...
    async getStats(): Promise<{
        ...
        if (!this.db) {
            return { count: 0, totalSize: 0, maxSize: this.config.maxSize, maxEntries: this.config.maxEntries };
        }
        ...
        return new Promise((resolve) => {
            ...
            request.onsuccess = () => {
                ...
                const totalSize = entries.reduce(
                    (sum, entry) => sum + (entry.metadata?.size || 0),
                    0
                );
            ...
            request.onerror = () => {
                resolve({ count: 0, totalSize: 0, maxSize: this.config.maxSize, maxEntries: this.config.maxEntries });
            };
        });
    }
}
