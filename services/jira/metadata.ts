import type { JiraConfig, JiraProject, JiraFieldInfo, JiraCustomFieldOption, GetJiraFieldsOptions } from './types';
...
export const getJiraProjects = async (config: JiraConfig, useCache: boolean = true): Promise<JiraProject[]> => {
    ...
    try {
        ...
        if (Array.isArray(response.values)) {
            ...
            logger.info(`Encontrados ${(response as JiraProject[]).length} projetos (formato alternativo)`, 'jiraService');
