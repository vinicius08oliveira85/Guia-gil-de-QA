...
export const syncJiraProject = async (
  ...
  const statusMapAntes = new Map<
    string,
    { taskId: string; testCaseId: string; expectedStatus: TestCase['status'] }
  >();
  ...
  return {
    ...
    settings: {
      ...
      jiraStatuses:
        mergedJiraStatuses.length > 0 ? mergedJiraStatuses : projectToUse.settings?.jiraStatuses,
    },
  };
};
