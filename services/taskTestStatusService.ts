...
if (supabaseUrl && supabaseAnonKey) {
  ...
  if (!isProduction()) {
    void Promise.race([
      ...
      .catch(error => {
        logger.debug(
          'Falha ao autenticar anonimamente no Supabase',
          'taskTestStatusService',
          error
        );
      });
...
const saveThroughProxy = async (
  ...
  try {
    ...
    if (retryCount === 0) {
      logger.debug(
        `Status de teste "${status}" salvo via proxy para ${taskKey}`,
        'taskTestStatusService'
      );
    }
    clearSupabaseRemotePause();
...
const saveThroughSdk = async (
  ...
  const savePromise = supabase.from('task_test_status').upsert(
    {
      task_key: taskKey,
      status: status,
  ...
  if (retryCount === 0) {
    logger.debug(
      `Status de teste "${status}" salvo via SDK para ${taskKey}`,
      'taskTestStatusService'
    );
  }
  clearSupabaseRemotePause();
 ...
 */
const loadMultipleThroughProxy = async (
  taskKeys: string[]
): Promise<Map<string, TaskTestStatus>> => {
...
const loadThroughSdk = async (taskKey: string): Promise<TaskTestStatus | null> => {
  ...
  try {
    ...
    logger.debug(
      'Falha ao carregar status via SDK (retornando null)',
      'taskTestStatusService',
      error
    );
    return null;
...
const loadMultipleThroughSdk = async (taskKeys: string[]): Promise<Map<string, TaskTestStatus>> => {
  ...
  try {
    ...
    if (isNetworkError(error)) {
      logger.warn(
        'Erro de rede ao carregar múltiplos status via SDK',
        'taskTestStatusService',
        error
      );
    } else {
...
export const loadTaskTestStatus = async (taskKey: string): Promise<TaskTestStatus | null> => {
  ...
  if (hasProxy) {
    try {
      ...
      logger.debug(
        'Falha ao carregar status via proxy (retornando null)',
        'taskTestStatusService',
        error
      );
      pauseSupabaseRemoteAfterTransientFailure();
 ...
 */
export const loadMultipleTaskTestStatus = async (
  taskKeys: string[]
): Promise<Map<string, TaskTestStatus>> => {
  ...
  if (hasProxy) {
    try {
      ...
      logger.debug(
        'Falha ao carregar múltiplos status via proxy (retornando vazio)',
        'taskTestStatusService',
        error
      );
      pauseSupabaseRemoteAfterTransientFailure();
 ...
 */
export const saveTaskTestStatus = async (
  taskKey: string,
  status: TaskTestStatus
): Promise<void> => {
  if (forceLocalOnly) {
  ...
  if (hasProxy) {
    try {
      ...
      logger.debug(
        'Falha ao salvar status via proxy. Mantendo apenas localmente.',
        'taskTestStatusService',
        error
      );
      pauseSupabaseRemoteAfterTransientFailure();
  ...
  if (hasSdk) {
    try {
      ...
      logger.debug(
        'Falha ao salvar status via SDK. Mantendo apenas localmente.',
        'taskTestStatusService',
        error
      );
      return;
  ...
  logger.debug(
    'Supabase não configurado. Status salvo apenas localmente.',
    'taskTestStatusService'
  );
};
